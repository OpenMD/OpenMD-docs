...
\title{{\sc OpenMD-3.0: Molecular Dynamics in the Open}}

\author{Cody Drisko, Sydney Shavalier,  Benjamin Harless, Veronica Freund, \\
  Teng Lin,  Charles F. Vardeman II, Christopher J. Fennell,  Matthew  A. Meineke, \\
  Patrick Louden,   Hemanta Bhattarai,   Joseph Michalka,  Kelsey  Stocker, \\
  James Marr,  Anderson D.S. Duraes,   Suzanne Neidhart,   Shenyu Kuang,  \\
...
\begin{figure}[H]
    ...
    \caption{\label{fig:SPF} Schematic showing the main steps in the Scaled Particle Flux RNEMD method. A particle in the source bin (A) and a random location in the sink (B) are selected. In the particle scaling loop, all atomic forces using both placements of the particle are computed ($\mathbf{f}_\mathrm{A}$ and $\mathbf{f}_\mathrm{B}$). Forces are combined using the scaling function, $\mathbf{f}(\lambda) = (1-s(\lambda))~\mathbf{f}_\mathrm{A} + s(\lambda)~ \mathbf{f}_\mathrm{B}$, and the system is propagated using standard MD integration with the combined forces. After a successful step, the particle progress variable, $\lambda$ is incremented by $d\lambda$.} 
\end{figure}
...
directional exchange, and {\tt particleFlux} for SPF simulations.
...
of the following: {\tt KE, Px, Py, Pz, Pvector, Lx, Ly, Lz, Lvector, Particle, Particle+KE, KE+Px, KE+Py, KE+Pvector, KE+Lx, KE+Ly, KE+Lz, KE+Lvector} \\
{\tt kineticFlux} & kcal mol$^{-1}$ \AA$^{-2}$ fs$^{-1}$ & specify the kinetic energy flux &  \\
{\tt momentumFlux} & amu \AA$^{-1}$ fs$^{-2}$ & specify the momentum flux & \\
{\tt particleFlux} & \AA$^{-2}$ fs$^{-1}$ & specify the particle flux & \\  
...
{\tt privilegedAxis} & string & in periodic boundaries, perform exchanges along which axis? & one
of the following: {\tt x, y, z} \\
...
file where each column is separated by a pipe ($\mid$) symbol. & Allowed column names are: {\sc z, r, temperature, velocity, angularvelocity, density, activity, electricfield, electrostaticpotential} \\
...
simulation cell along the $z$-axis.  The first column of the {\tt
  .rnemd} file is the $z$ coordinate of the center of each bin, while
following columns may contain the average temperature, velocity,
density, or activities (concentrations) within each bin.  The output
format in the {\tt .rnemd} file can be altered with the {\tt
  outputFields}, {\tt outputBins}, and {\tt outputFileName}
parameters.  A report at the top of the {\tt .rnemd} file contains the
current exchange totals as well as the average flux applied during the
simulation.  Using the slope of the temperature or velocity gradient
obtaine from the {\tt .rnemd} file along with the applied flux, the
user can very simply arrive at estimates of thermal conductivities
($\lambda$),
\begin{equation}
...
care. Here temperature jumps across the interface are required to
...
the heat rate, $q_r$ and to compute interfacial thermal conductance
from the total Kapitza resistance across a finite-width interfacial
region, 
\begin{equation}
...
We suggest that users interested in interfacial conductance consult
references
\citealp{kuang:AuThl,Stocker:2016kx,Bhattarai:2020lp,Shavalier:2022kn},
and \citealp{Shavalier:2023rm}
for other approaches to computing $G$.  Users interested in {\it
  friction coefficients} at heterogeneous interfaces may also find
references \citealp{2012MolPh.110..691K,Louden:2017tc}, and \citealp{Louden:2018le} useful.
...
{\tt  -Hxxx} & (xxx is file format ID e.g. -Hpdb) gives format info \\
...
Note that Hydro also generates molecular Pitch matrices.
...
{\tt affineScale} is a utility script which takes an {\sc OpenMD}
