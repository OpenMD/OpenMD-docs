...
\newcolumntype{F}{p{1.75in}}
\newcolumntype{G}{p{3.5in}}
...
&  {\tt -{}-velocityZ}               & two-dimensional velocity map \\
 ...
 &  {\tt -{}-countz}                 & computes the number of selected atoms  along
                                  selected axis\\
  -M&  {\tt -{}-momentum\_distribution} &  momentum distribution \\
  -S&  {\tt -{}-dipole\_orientation}     & spatially-resolved dipole order parameter $S(z)$,
                                  $S = (3 \cos^2\theta - 1)/2$ \\
 &  {\tt -{}-order\_prob}            &  probability of order parameter for given
                                  selection
...
\hline
  ...
  -v& {\tt -{}-vcorr}                  & velocity autocorrelation
    ...
    & {\tt -{}-vcorrR}                 & velocity correlation function projected
                                         radially\\
    & {\tt -{}-vaOutProdcorr}           & Velocity - Velocity auto outer product
                                  correlation function\\
    & {\tt -{}-waOutProdcorr}           & Angular Velocity - Angular Velocity auto outer
                                  product correlation function\\
    & {\tt -{}-vwOutProdcorr}           & Velocity - Angular Velocity outer product
                                  correlation function\\
    & {\tt -{}-wvOutProdcorr}           & Angular Velocity - Velocity outer product
                                  correlation function\\
  ...
  -j& {\tt -{}-jumptime}               & Hydrogen bond jump time
    ...
    & {\tt -{}-jumptimeZ}               & Hydrogen bond jump time correlation function
                                  binned by Z \\
    & {\tt -{}-jumptimeR}               & Hydrogen bond jump time correlation function
    ...
    & {\tt -{}-ddisp}                  & Collective Dipole displacement function
                                  (Helfand moment of Current Density)\\
     & {\tt -{}-rotAngleDisp}           & Displacement correlation function for rotation
                                  angles\\
...
\endfoot
  -h& {\tt -{}-help}               & Print help and exit\\
  -v& {\tt -{}-version}          & print version and exit \\
  -i& {\tt -{}-input}        & use specified input file name
                               (mandatory) \\
  -b& {\tt -{}-box}          &  Optimize box geometry before performing calculation
                         (default=off) \\
  -m& {\tt -{}-method=\{``energy'',``stress''\}} & Calculation Method \\
  -n& {\tt -{}-npoints} &   number of points for fitting
                         stress-strain relationship  (default=25) \\
  -d& {\tt -{}-delta}  & size of relative volume changes for strains
\end{longtable}
...
\endfoot
  ...
  -v& {\tt -{}-vacuum}             & should the output (.omd) file
                                     have vacuum in the z-dimension?
                                     (true / false) \\
  -d& {\tt -{}-directional}        & Output atoms with orientational
...
\endfoot
  -h& {\tt -{}-help}               & Print help and exit\\
  -V& {\tt -{}-version}            & Print version and exit\\
  -i& {\tt -{}-input=filename}     & Input (omd) file\\
  -o& {\tt -{}-output=STRING}      & Output file prefix (mandatory)\\

\hline
\multicolumn{3}{|l|}{One option from the following group of options is required:} \\
\hline
  -t& {\tt -{}-temperature=DOUBLE}     & temperature (K) \\
  -e& {\tt -{}-energy=DOUBLE}          & energy (kcal/mol) \\
  -c& {\tt -{}-chargetemperature=DOUBLE}
...
\endfoot
  -h& {\tt -{}-help}               & Print help and exit\\
  -V& {\tt -{}-version}            & Print version and exit\\
  -i& {\tt -{}-input=filename}     & input MetaData (omd) file\\
  -x& {\tt -{}-xyz=filename}       & xyz file for AtomicBead model\\
    & {\tt -{}-stl=filename}       & stl file for BoundaryElement model\\
    & {\tt -{}-msms=filename}      & filename root for MSMS .vert and .face files\\
  -o& {\tt -{}-output=STRING}      & output file prefix (default = `hydro')\\
    &  {\tt -{}-model=\{``AtomicBead'', ``RoughShell'', or ``BoundaryElement''\}}  & hydrodynamics
                                                       model, mandatory \\
  -s& {\tt -{}-beadSize=DOUBLE}    & bead size for RoughShell model in angstroms
                           (default=0.2) \\
  -b&  {\tt -{}-elements}          & output the hydrodynamic elements (beads or
                              triangles) only, hydrodynamics calculation will
                                       not be performed  (default=off)\\
  -v& {\tt -{}-viscosity=DOUBLE}   & viscosity (in poise)  (default=`0.01')\\
  -t& {\tt -{}-temperature=DOUBLE} & temperature (in Kelvin  (default=`300')
\end{longtable}
...
\endfoot
  ...
  -c               & default overlap cutoff for spacing water
                     molecules (default 3.3) \\
